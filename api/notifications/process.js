...
const SUPPORTED_VARIABLES = new Set(["student_first_name", "student_full_name", "student_registration_number", "instructor_name", "previous_instructor_name", "new_instructor_name", "lesson_number", "lesson_title", "class_date", "class_time", "programme_name", "dashboard_link", "submission_link", "question_link", "class_link", "certificate_link"]);
const MAX_ITEMS = 25;
...
export default async function handler(req, res) {
  ...
  for (const item of queue || []) {
    try {
      ...
      if (!rule || !recipient || recipient.status !== "active" || !recipient.email) throw new Error("Recipient or automation rule is unavailable.");
